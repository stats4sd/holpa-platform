...
class IsLookupListResource
{
    ...
    // do not use tenancy by default, but instead filter by tenancy + add in items with team_id === null (i.e. global items)
    ...
    public static function table(Table $table): Table
    {
        ...
        if (static::getModel()::canBeHiddenFromContext()) {
            $actions[] = Action::make('Toggle Removed')
                ...
                ->label(fn(LookupListEntry $record): string => $record->isRemoved(HelperService::getCurrentTenant()) ? 'Restore to Context' : 'Remove from Context')
                ->action(fn(LookupListEntry $record) => $record->toggleRemoved(HelperService::getCurrentTenant()));
        }
    ...
    {
        $team = HelperService::getCurrentTenant();
